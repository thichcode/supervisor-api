...
from src.core.schemas import UserInfo, ConversationInfo, CaseInfo, MessageInfo
...
async def readiness_check():
    ...
    try:
        if await redis_cache.exists("health_check"):
            checks["redis"] = True
        else:
...
async def receive_webhook(
    ...
    try:
        if settings.webhook_input_secret and x_webhook_secret != settings.webhook_input_secret:
        ...
        payload.message.text = sanitized_text

        start_time = time.time()
        request_logger.log_request_received({
            "user": {"id": payload.user.id},
            "conversation": {"thread_id": payload.conversation.thread_id}
        })

        from src.db import async_session

        async with async_session() as session:
            memory_service = MemoryService(session, redis_cache)
            memory = await memory_service.retrieve(payload)

            result = await supervisor.process(payload, memory)
